...
export const Filter = ({
    ...
    const updateFilter = (id: string, updates: Partial<FilterValue>) => {
        setActiveFilters(prev => prev.map(f => {
            ...
            if (updates.field) {
                ...
                if (newField) {
                    updated.operator = DEFAULT_OPERATORS[newField.type][0].value;
                    updated.value = newField.options ? (newField.options[0] as string) : '';
                }
            }
            return updated;
        }));
    ...
    const fieldOptions: SelectOption<string>[] = Object.entries(fields).map(([key, cfg]) => ({
        value: key,
        label: cfg.label
    }));
                ...
                <div className="p-5">
                    <div className="space-y-3">
                        {activeFilters.map((filter) => {
                            ...
                            const operators = DEFAULT_OPERATORS[fieldConfig.type];
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="shrink-0 w-full md:w-48">
                                        <Select
                                            value={filter.field}
                                            onChange={(val) => updateFilter(filter.id, { field: val as string })}
                                    ...
                                    <div className="shrink-0 w-full md:w-40">
                                        <Select
                                            ...
                                            onChange={(val) => updateFilter(filter.id, { operator: val as string })}
                                    ...
                                    {!currentOp?.noValue && (
                                        <div className="grow min-w-[200px]">
                                            {fieldConfig.type === 'select' && fieldConfig.options ? (
                                                <Select
                                                    value={filter.value}
                                                    onChange={(val) => updateFilter(filter.id, { value: val as string })}
                                                ...
                                                <Input
                                                    ...
                                                    placeholder={fieldConfig.type === 'date' ? '' : "Suchbegriff..."}
                                                />
                                            )}
                                        </div>
                                    )}
