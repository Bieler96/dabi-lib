	...
	return (
		<div className="p-8">
			...
			<div className="flex flex-col gap-4 items-start">
				<Button
					variant="filled"
					onClick={() => nav.navigate("details", { id: 42 })}
				>
					Go to Router Demo
				</Button>
				<Button
					variant="outlined"
					onClick={() => nav.navigate("sheet-demo")}
				>
					Go to Sheet Demo
				</Button>
				<Button
					variant="outlined"
					onClick={() => nav.navigate("user-list")}
				>
					Go to User List (Router Type)
				</Button>
				<ButtonGroup>
					<Button
						variant="tonal"
						onClick={() => nav.navigate("datatable-demo")}
					>
						Go to DataTable Demo
					</Button>
					<ButtonGroupSeparator />
					<Button
						variant="tonal"
						onClick={() => nav.navigate("jira-board")}
					>
						Go to Jira Clone
					</Button>
				</ButtonGroup>
				<Button
					variant="outlined"
					onClick={() => nav.navigate("sidebar-demo")}
				>
					Go to Side Menu Demo
				</Button>
				<Button
					variant="outlined"
					onClick={() => nav.navigate("guard-demo")}
				>
					Go to Guard Demo
				</Button>
				<Button
					variant="tonal"
					onClick={() => nav.navigate("filter-demo")}
				>
				...
				<Button
					...
					onClick={() => nav.navigate("user-management")}
				>
	...
	return (
		<div className="p-8">
			...
			<div className="grid grid-cols-2 gap-4 max-w-md">
				<Button variant="tonal" onClick={() => setActiveSheet("left")}>
					Open Left
				</Button>
				<Button variant="tonal" onClick={() => setActiveSheet("right")}>
					Open Right
				</Button>
				<Button variant="tonal" onClick={() => setActiveSheet("top")}>
					Open Top
				</Button>
				<Button
					variant="tonal"
					onClick={() => setActiveSheet("bottom")}
				>
					Open Bottom
				</Button>
			</div>
			...
			>
				<div className="py-4">
					<p className="text-on-surface-variant">
						Content for the {activeSheet} sheet goes here. You can
						put anything inside!
					</p>
	...
	return (
		<div className="flex flex-col gap-2 p-2">
			...
			<div className="flex flex-row gap-2 justify-end mt-4">
				...
				<Button variant="filled" onClick={nav.popBackStack}>
					Löschen
				</Button>
			</div>
		</div>
		...
		<NavHost
			...
			builder={(nav) => {
				...
				nav.list("user-list", {
					title: "Users",
					description:
						"List of all registered users managed via Router 'list' type.",
					columns: userColumns,
					data: users,
				});
				...
				nav.screen("protected-page", ProtectedPage, {
					canActivate: [authGuard],
				});
				nav.screen("dirty-page", DirtyPage, {
