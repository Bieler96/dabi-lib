	...
	return (
		<div className="p-8 max-w-7xl mx-auto space-y-8">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-on-surface-variant mt-2">
						Eine flexible Filter-Komponente für komplexe
						Datenstrukturen.
					</p>
				</div>
			...
			<Filter
				...
				onApply={handleApply}
				initialFilters={[
					{ id: "1", field: "status", operator: "=", value: "Neu" },
				]}
			/>
						...
						<div className="flex flex-wrap gap-2 min-h-[40px]">
							{query && query.length > 0 ? (
								query.map((f) => (
									<div
										key={f.id}
										className="inline-flex items-center gap-2 bg-primary/10 text-primary px-3 py-1.5 rounded-lg text-xs border border-primary/20"
									>
										<span className="font-semibold text-on-surface">
											{FIELDS[f.field]?.label || f.field}
										</span>
