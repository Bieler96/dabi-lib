		...
		<div className="space-y-6">
			...
			<div className="pt-4 flex flex-col gap-2">
				<Button
					variant="filled"
					className="w-full"
					onClick={() => nav.navigate("edit-profile", { user })}
				>
					Profil Bearbeiten
				</Button>
				<Button
					...
					onClick={() => nav.navigate("block-user", { user })}
				>
					Benutzer Sperren
				</Button>
			</div>
	...
	return (
		<div className="p-2">
			...
			<p className="text-on-surface-variant mb-6">
				Sind Sie sicher, dass Sie <strong>{user.name}</strong> sperren
				möchten? Der Benutzer wird keinen Zugriff mehr auf das System
				haben.
			</p>
		...
		},
		{
			...
			cell: ({ row }) => (
				<Button
					variant="ghost"
					size="icon"
					onClick={() => nav.navigate("user-details", { user: row })}
				>
		...
		<div className="p-8 max-w-6xl mx-auto">
			<div className="flex items-center justify-between mb-8">
				<div>
					...
					<p className="text-on-surface-variant">
						Ein Beispiel für die Verwendung der DataTable
						Komponente.
					</p>
				</div>
			...
			<section className="mb-12">
				<h2 className="text-xl font-semibold mb-4 text-on-surface">
					1. Manuelle Spaltendefinition
				</h2>
				<p className="text-sm text-on-surface-variant mb-4">
					Hier werden Spalten explizit definiert mit Custom-Rendering
					für Status, Rollen und Aktionen.
				</p>
				<DataTable
					data={mockUsers}
					columns={columns}
			...
			<section className="mb-12">
				<h2 className="text-xl font-semibold mb-4 text-on-surface">
					2. Automatische Spaltenerkennung
				</h2>
				<p className="text-sm text-on-surface-variant mb-4">
					Wenn keine Spalten definiert sind, extrahiert die Komponente
					diese automatisch aus den Daten. Verschachtelte Objekte
					werden ebenfalls erkannt.
				</p>
				<DataTable data={mockUsers} headerAlignment={Alignment.LEFT} />
			</section>

			<section>
				<h2 className="text-xl font-semibold mb-4 text-on-surface">
					3. Andere Ausrichtungen
				</h2>
				<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
					<div>
						<h3 className="text-sm font-medium mb-2">
							Zentriert (Alignment.CENTER)
						</h3>
						<DataTable
