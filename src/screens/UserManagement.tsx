...
const columns: ColumnDef<User>[] = [
	...
	},
	{
		...
		header: "Name",
		cell: ({ row }) => {
			return <CopyButton text={row.name} label={row.name} />;
		},
	},
	{
		accessorKey: "email",
		header: "Email",
		cell: ({ row }) => {
			return <CopyButton text={row.email} label={row.email} />;
		},
	},
	// {
...
const getColumns = ({
	onEdit,
	onDelete,
}: UserManagementProps): ColumnDef<User>[] => [
	...columns,
	{
		accessorKey: "actions",
		header: "Actions",
		...
		try {
			const params = new URLSearchParams({
				...
				...(activeFilters.before && {
					createdAtBefore: activeFilters.before,
				}),
				...(activeFilters.after && {
					createdAtAfter: activeFilters.after,
				}),
			});
			...
			<div className="flex justify-between items-center">
				<div>
					...
					<p className="text-on-surface-variant">
						Fullstack example with Drizzle & SQLite
					</p>
				</div>
				<div className="flex gap-2">
					<Button
				...
				<div className="flex justify-center mt-4">
					<Pagination>
						<PaginationContent>
							...
							{[
								...Array(
									Math.max(1, metadata?.totalPages || 0),
								),
							].map((_, i) => (
